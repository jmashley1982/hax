...
## History note
This repo previously used an auto-generated `claude/...` branch as its
default. `main` was created from that branch's tip on 2026-08-16 and is
now the trunk. If you still see a stale `claude/...` branch, ignore it.
...
Binary image commits through the GitHub API are unreliable here. If one
fails, say so plainly rather than retrying forever.
